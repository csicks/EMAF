...
public:
    ...
    explicit stackReal(const long long *s) {
        shape[0] = s[0];
        shape[1] = s[1];
        shape[2] = s[2];
    ...
    T get(long long i, long long j, long long k) const {
        return data[i * shape[1] * shape[2] + j * shape[2] + k];
    }

    void set(long long i, long long j, long long k, T value) {
        data[i * shape[1] * shape[2] + j * shape[2] + k] = value;
    }

    imageReal<T> pieceGet(long long index) const {
        long long imgShape[2] = {shape[1], shape[2]};
        imageReal<T> img = imageReal<T>(imgShape);
        img.data.set(0, data.get(index * shape[1] * shape[2], shape[1] * shape[2]));
        return img;
    }

    void pieceSet(long long index, const imageReal<T> &img) {
    ...
    stackReal<T> pieces(int start, int end) {
        long long s[3] = {end - start + 1, shape[1], shape[2]};
...
stackReal<T> image2Stack(const imageReal<T> &img) {
    long long stkShape[3] = {1, img.shape[0], img.shape[1]};
...
stackReal<T> image2Stack(const imageReal<T> img[], int l) {
    long long stkShape[3] = {l, img[0].shape[0], img[0].shape[1]};
...
public:
    ...
    explicit stackComplex(const long long *s);

    stackComplex(const stackComplex &stk);

    double *get(long long i, long long j, long long k) const;

    void set(long long i, long long j, long long k, const double value[2]);

    imageComplex pieceGet(long long index) const;

    void pieceSet(long long index, const imageComplex &img);
