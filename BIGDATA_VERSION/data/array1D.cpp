...
arrayComplex::arrayComplex(const double d[][2], long long l) {
...
arrayComplex::arrayComplex(long long l) {
    maxLength = l > maxLengthInit ? l : maxLengthInit;
    data = new double[2 * maxLength];
    for (long long i = 0; i < 2 * l; ++i)
        data[i] = 0;
    length = l;
}
...
double *arrayComplex::get(long long index) const {
    auto *r = new double[2];
    r[0] = data[2 * index];
    r[1] = data[2 * index + 1];
    return r;
...
arrayComplex arrayComplex::get(long long index, long long l) const {
    if (index + l > length)
...
void arrayComplex::set(long long index, const arrayComplex &ary) {
...
void arrayComplex::set(long long index, const double value[2]) {
    if (index > length - 1)
        throw baseException("Error: Index out of range!");
...
double *arrayComplex::operator[](long long index) const {
    auto *r = new double[2];
    r[0] = data[2 * index];
    r[1] = data[2 * index + 1];
    return r;
...
arrayComplex arrayComplex::operator*(double value) const {
    arrayComplex ary = arrayComplex(this);
    for (long long i = 0; i < 2 * length; ++i)
        ary.data[i] *= value;
    return ary;
}
...
arrayComplex bind(const arrayReal<double> &abs, const arrayReal<double> &angle) {
    ...
    for (long long i = 0; i < r.length; ++i) {
