...
public:
    ...
    arrayReal<T> get(int index, int l) const {
        if (index + l > length)
    ...
    void set(int index, const arrayReal<T> &ary) {
...
public:
    ...
    arrayComplex(const double d[][2], int l);

    ///zeros
    explicit arrayComplex(int l);
    ...
    arrayComplex get(int index, int l) const;

    void set(int index, const arrayComplex &ary);

    void set(int index, const double value[2]);
