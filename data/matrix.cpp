...
matrix::matrix(const int s[2]) {
    shape[0] = s[0];
    shape[1] = s[1];
    data = arrayReal<double>(static_cast<size_t>(s[0]) * s[1]);
}
...
matrix::matrix(const double *d, const int s[2]) {
    data = arrayReal<double>(d, static_cast<size_t>(s[0]) * s[1]);
    shape[0] = s[0];
    shape[1] = s[1];
}
...
arrayReal<double> matrix::operator[](int index) const {
    ...
    memcpy(ary.data, data.data + static_cast<size_t>(index) * shape[1], sizeof(double) * shape[1]);
    return ary;
}
...
matrix matrix::removeRow(int index) const {
    ...
    memcpy(temp.data.data, data.data, static_cast<size_t>(index) * shape[1] * sizeof(double));
    memcpy(temp.data.data + static_cast<size_t>(index) * shape[1],
           data.data + static_cast<size_t>(index + 1) * shape[1],
           static_cast<size_t>(shape[0] - index - 1) * shape[1] * sizeof(double));
    return temp;
}
...
matrix matrix::operator+(const matrix &matrixIn) const {
    if (shape[0] != matrixIn.shape[0] or shape[1] != matrixIn.shape[1])
