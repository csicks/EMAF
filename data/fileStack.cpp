...
fileStack::fileStack(const std::string &path, const std::string &extension, const std::string &aux,
    ...
    mrcFile mrc = mrcFile();
    mrc.read(names[0]);
    shape[0] = static_cast<int>(names.size());
    shape[1] = mrc.data.shape[1];
    shape[2] = mrc.data.shape[2];
}

double fileStack::get(int i, int j, int k) const {
    imageReal<double> img = pieceGet(i);
    return img.get(j, k);
}

imageReal<double> fileStack::pieceGet(int index) const {
    mrcFile mrc = mrcFile();
    mrc.read(names[index]);
    imageReal<double> img = mrc.data.pieceGet(0);
    return img;
}

void fileStack::pieceSet(int index, const imageReal<double> &img) {
...
void fileStack::append(const imageReal<double> &img) {
    if (shape[0] == 0) {
        shape[1] = img.shape[0];
        shape[2] = img.shape[1];
    }
