...
arrayComplex::arrayComplex(const double d[][2], size_t l) {
    length = l;
...
arrayComplex::arrayComplex(size_t l) {
    length = l;
    maxLength = l > maxLengthInit ? l : maxLengthInit;
    data = new double[2 * maxLength];
...
double *arrayComplex::get(size_t index) const {
    auto *r = new double[2];
    r[0] = data[2 * index];
    r[1] = data[2 * index + 1];
    return r;
...
arrayComplex arrayComplex::get(size_t index, size_t l) const {
    if (index + l > length)
...
void arrayComplex::set(size_t index, const arrayComplex &ary) {
    if (index + ary.length > length)
        throw baseException("Error: Index out of range!");
    memcpy(data + 2 * index, ary.data, 2 * ary.length * sizeof(double));
}

void arrayComplex::set(size_t index, const double value[2]) {
    if (index > length - 1)
        throw baseException("Error: Index out of range!");
...
double *arrayComplex::mean() const {
    auto *r = new double[2];
    for (size_t i = 0; i < length; ++i) {
        r[0] += data[2 * i] / static_cast<double>(length);
        r[1] += data[2 * i + 1] / static_cast<double>(length);
    }
    return r;
}

double *arrayComplex::operator[](size_t index) const {
    auto *r = new double[2];
    r[0] = data[2 * index];
    r[1] = data[2 * index + 1];
    return r;
